...
`.gmail-token.json` must contain OAuth credentials JSON generated by `npm run auth`. A Google API key is not enough for Gmail read/modify access.
...
If the server appears connected but tools return nothing, call `gmail_auth_status` first. It checks whether `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, and the OAuth token file are available without printing secrets.
